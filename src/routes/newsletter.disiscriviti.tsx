/**
 * /newsletter/disiscriviti — Landing page per token-based unsubscribe
 *
 * Loader pattern: useEffect on mount → POST /api/newsletter/unsubscribe con token.
 * Mostra loading/success/error states.
 *
 * URL: /newsletter/disiscriviti?token=XXX
 */
...
export const Route = createFileRoute("/newsletter/disiscriviti")({
  validateSearch,
    ...
    fetch("/api/newsletter/unsubscribe", {
      ...
      .then(async (res) => {
        ...
        if (res.ok) {
          ...
          setMessage(
            json?.data?.message ??
              "Sei stato disiscritto dalla newsletter. Ci dispiace vederti andare.",
          );
        } else {
          setState("error");
          setMessage(json?.error?.message ?? "Errore durante la disiscrizione");
        }
      })
      .catch(() => {
        setState("error");
  ...
  return (
    <div className="mx-auto max-w-lg px-[var(--page-padding-x)] py-24 text-center">
      ...
      <h1 className="mt-6 font-display text-2xl font-semibold text-[var(--color-text)]">
        {state === "loading" && "Disiscrizione in corso..."}
        {state === "success" && "Disiscrizione completata"}
        {state === "error" && "Disiscrizione non riuscita"}
      </h1>
